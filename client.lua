...
NyoModules.registerNyoModule(module, function()
    ...
    NyoModulesClient.registerNUICallback('garageClose', function(data,cb)
        ...
        TriggerServerEvent('nyo_modules:close_garage')
        inShop = false
        UIOpen = false
    ...
    NyoModulesClient.registerNUICallback('spawn_vehicle', function(data,cb)
            ...
            if not spawnCoords then
                cb({success = false})
                TriggerEvent("Notify","negado","Todas vagas estão ocupadas")
    ...
    RegisterCommand("nyo_modules:vehicleLock2", function()
        local ped = plyPed
        local cds = plyCoords
        ...
        if veh and veh ~= 0 then
			...
			if vidas > 101 then
                ...
                local success = RPC.trigger("nyo_modules:setVehicleLock", VehToNet(veh), vehLock == 1 and 2 or 1,plate)
    ...
    RegisterCommand("chave", function(source,args,rawCommand)
        ...
        if veh and veh ~= 0 then
            ...
            if args[1] == "add" and args[2] then
                RPC.trigger("nyo_modules:chave", VehToNet(veh),args[2],plate)
            elseif args[1] == "remove" then
                RPC.trigger("nyo_modules:remove", VehToNet(veh),plate)
            end
        end
    end)

    RegisterKeyMapping("nyo_modules:vehicleLock2", 'Trancar / Destrancar veículos', 'KEYBOARD', "l")
    ...
    exports('isVehicleBanned', garages.isVehicleBanned)
     ...
     -- Home Garages 
     SafeEvents.register('nyo_modules:createHomeGaragem', function(gData)
